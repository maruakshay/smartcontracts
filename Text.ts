...
const json = require('./build/contracts/sharingFile.json')
...
const contract = new web3.eth.Contract(list.abi, list.address);

contract.methods.getText().call().then((text:string) => console.log(`text is  ${sha256(text).toString()}`));

web3.eth.getAccounts().then(accounts => {
    var acc1 = accounts[1];
    console.log(`The account is ${acc1}`);
    return contract.methods.getText().send({from : acc1}).then((data:object) => console.log(data))
})
